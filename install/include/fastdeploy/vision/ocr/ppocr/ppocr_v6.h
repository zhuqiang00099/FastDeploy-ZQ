 ...
 *
 * PP-OCRv6 shares the same pre/post-processing code path as PP-OCRv5 in
 * PaddleOCR cpp_infer (only the model weights differ), so PPOCRv6 simply
 * inherits from PPOCRv5 without changing any preprocessing parameters.
 */
class FASTDEPLOY_DECL PPOCRv6 : public PPOCRv5 {
 public:
   ...
   *
   * \param[in] det_model Path of detection model, e.g ./PP-OCRv6_medium_det_infer
   * \param[in] cls_model Path of classification model, e.g ./ch_ppocr_mobile_v2.0_cls_infer
   * \param[in] rec_model Path of recognition model, e.g ./PP-OCRv6_medium_rec_infer
   */
